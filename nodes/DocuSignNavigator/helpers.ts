...
function getBaseUrl(environment: string, accountId: string): string {
  const base =
    environment === 'demo'
      ? 'https://api-d.docusign.com/navigator/v1'
      : 'https://api.docusign.com/navigator/v1';
  return `${base}/accounts/${accountId}`;
}

export async function navigatorApiRequest(
  ...
  const credentials = await this.getCredentials('docuSignNavigatorApi');
